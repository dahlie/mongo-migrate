...
function getMigrationsCollection(db, mongodb, collectionName) {
	// for mongodb 2.x
	if (typeof db.collection !== 'undefined') {
		return db.collection(collectionName || 'migrations');
	}
...
function getConnection(opts, cb) {
	...
	if (opts.connectionString) {
		...
		MongoClient.connect(opts.connectionString, function(err, db) {
			...
			cb(null, {
				connection: db,
				migrationCollection: getMigrationsCollection(db, mongodb, opts.collection)
			});
		});
		return;
	}
